...
char	*chpass ;
char	*npassname ;
char	pass0[64] = "/src/Machines/16032/c16/c16";
char	pass1[64] = "/lib/o16";
char	pass2[64] = "/lib/xxx";
char	passp[64] = "/lib/cpp";
char	libdr[64];
char	*ldrel = NULL;	/* -T argument for loader */
# ifndef nsc16000	/* PDP11 or VAX11/780	*/
char	pref[64]  = "/src/Machines/16032/lib/crt0.b";
char	incld[64] = "-I/src/Machines/16032/include";
char	*a16 = "/src/Machines/16032/a16/a16";
char	*ld16 = "/src/Machines/16032/cmd/ld16";
# else			/* 16000 */
char	pref[64]  = "/src/Machines/16032/lib/crt0.b";
...
passa:
	...
	*pv++ = "-Dnsc16000";
	...
	for (i=0; i<nc; i++) {
		if (nc>1)
			printf("%s:\n", clist[i]);
		if (strcmp(getsuf(clist[i]), "a16") == 0) {
			assource = clist[i];
			goto assemble;
		} 
		else
		...
		av[j++] = 0;
		if (sflag)
		    assource = tmp3 = setsuf(clist[i], "a16");
...
char *as;
{
	...
	while (*tsp++ = *s++)
		;
	tsp += 4;	/* really only need 2 ; hack for things like .a16 */
	if (tsp > tsa+CHSPACE) {
