...
/* Sym_Size -	Compute size of symbol table segment of output file */
Sym_Size()
  {	register struct sym_bkt  **sbp1, *sbp2;
	long size = 0;
...
long Sym_Write(file)
  ...
  {	register struct sym_bkt  **sbp1, *sbp2;
	register char *sp;
	long slength = sizeof(slength);
	struct nlist s;

	fseek(file, (long)(N_SYMOFF(filhdr)), 0);
	for (sbp1 = sym_hash_tab; sbp1 < &sym_hash_tab[HASH_MAX]; sbp1++)
	  for (sbp2 = *sbp1; sbp2; sbp2 = sbp2->next_s)
	    if (sbp2->id_s != -1) {
	      if (!(sbp2->attr_s&S_DEF1)) s.n_type = N_UNDF;
	...
	fseek(file, (long)(N_STROFF(filhdr)), 0);
	...
	fseek(file, (long)(N_TXTOFF(filhdr)), 0);
}
