...
sucomp( p ) register NODE *p; {
	...
	if( o==PLUS || o==MUL || o==OR || o==ER || o==AND ){
...
/* ***cjt in progress...
		if( (q=p->in.left)->in.op==LS
		    (q->in.left->in.type==INT || q->in.left->in.type==UNSIGNED)
		    && q->in.right->in.op==ICON
		    && q->in.right->in.name[0]=='\0'
		    && ((i=q->in.right->tn.lval)==2 || i==4 || i==8) ) {
		  if (q->in.right->in.op != REG)
		    order( q->in.right, INTAREG|INAREG );
		  return;
*/		}
