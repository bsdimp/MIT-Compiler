...
/* the followin line modified by Bill O'Farrell */
	printf("\tlea\tsp,*-4(bp)\n\tpop\tdi\n\tpop\tsi\n\tpop\tbp\n\treti\n\t.para\n");

	p2bend();
	}

/* the following function added by Bill O'Farrell */
bpcode(){
	/* code for the beginning of a program */
	locctr(DATA);
	printf("\t.comm\tcalltmp,4\n");
	}
...
/* Bill O'Farrell changed the following test to eliminate direct switches.  The
   switch table can probably be changed to implement direct switches on
   the 8086, but I havn't got time to fix this right now. */
	if( 0/*range>0 && range <= 3*n && n>=4*/ ){ /* implement a direct switch */
