...
struct ins_init { char *opstr; short opnum; } op_codes[] = {
	...
	".ascii", i_ascii,
	".zerow", i_zerow,
/* the following line added by Bill O'Farrell, June 28, 1982 */
	".para", i_para,
	0 };
...
{	register int i,j;
	...
	while (--argc > 0) {
	  if (argv[0][0] == '-') switch (argv[0][1]) {
	    ...
	    Source_name = argv[0];
/* .b86 in two lines below -- Bill O'Farrell */
	    Concat(File_name, argv[0], ".b86");
...
/* .b86 and .d in few lines below -- Bill O'Farrell */
	if(!O_outfile)
	{
		if ((end = rindex(Source_name, '.')) == 0 ||
			strcmp(end, ".b86") != 0)
			Concat(Rel_name,Source_name,".d");
		else	/* copy basename without .b86 to Rel_name */
		{
			for (cp1 = Source_name, cp2 = Rel_name; cp1 < end;)
				*cp2++ = *cp1++;
			strcpy(cp2, ".d");	/* append ".d" to basename */
		}
	}
