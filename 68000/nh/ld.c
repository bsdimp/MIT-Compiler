...
struct archdr archdr;			/* directory part of archive */
...
char *e22 = "symbol table overflow on calloc";
...
register char *cp;
{
	...
	case ARCMAGIC:
		...
		position = 2;
		fseek(text, 2L, 0);	/* skip magic word */
...
middle()
{
	...
	if (dflag || rflag==0)
	{
		...
		ldrsym(slookup("_ldtime"), time(0), EXTERN+TEXT);
		common();
	}
	/*
...
long rsize;		/* size of appropriate relocation data */
{
	...
	long offs = 0;			/* value of offset to use */
...
register char *cp;	/* name of the file to open */
{
	...
	if (cp[0]=='-' && cp[1]=='l') {
		...
		for(c=0; cp[c+2]; c++) filename[c+12] = cp[c+2];
		filename[c+12] = '.';
		filename[c+13] = 'a';
		filename[c+14] = '\0';
	...
	if (((short)(magic>>16)) == (short)ARCMAGIC)
...
register symp sp;
{
	...
	if (sp == NULL)
	{
		...
		if (symindex>=NSYM) {
		    error("current number of symbols is %d\n", NSYM);
		    fatal(e17);
		}
