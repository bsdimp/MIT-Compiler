...
*/
		...
		*tp = 0;
		for (ap = tp; ap >= tobuf; --ap)
		    if ((int)any(ap, MR_FSTOPPERS)) break;
		...
		ap = tp;
		if (ap > tobuf && *(ap-1) == '/' &&
		    ap-1 > tobuf && (int)any(ap-2, MR_STOPPERS)) {
			backup(fd_err, 1);
			tp--;
			break;
		}
		while (ap > tobuf && (int)any(ap-1, MR_STOPPERS)) {
		    backup(fd_err, 1);
		    ap--;
		}
		while (ap > tobuf && !(int)any(ap-1, MR_STOPPERS)) {
		...
		ap = tp;
		while (ap <= hp) {
		    if (!(int)any(ap, MR_STOPPERS)) break;
		    outn(fd_err, ap++, 1);
		}
		while (ap <= hp) {
		    if ((int)any(ap, MR_STOPPERS)) break;
