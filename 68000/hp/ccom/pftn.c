...
defid( q, class )  
register NODE *q; 		/* MFM */
register int class;		/* MFM */
{
	register struct symtab *p;
	register int temp;			/* MFM */
	register int scl;			/* MFM */
	register TWORD type;			/* MFM */
	TWORD stp;
	int idp, dsym, ddef;
	int slev;
...
# ifndef BUG1
	if( ddebug ){
		printf( "defid( %.*s (%x), ", NCHNAM, p->sname, idp );
		tprint( q->in.type );
...
# endif
	...
	if( class==MOU || class==MOS || class & FIELD ) { /* make a new entry */
		...
		for( memp = &paramstk[paramno-1];
			...
			/* iterate */ --memp){ register char * cname, * oname;
...
# endif
	...
	for( i = oparam+4;  i< paramno; ++i ){
		...
		if( sz == 0 ){
			uerror( "illegal zero sized structure member: %.*s", 
				NCHNAM, p->sname );
			}
...
ftnarg( idn ) {
	...
	case UNDEF:
		...
		break;
	case FARG:
		uerror("redeclaration of formal parameter, %.*s", NCHNAM, 
			stab[idn].sname);
		/* fall thru */
	case FTN:
...
inforce( n ) register OFFSZ n; {  /* force inoff to have the value n *//* MFM */
	/* inoff is updated to have the value n */
	register OFFSZ wb;			/* MFM */
	register rest;
...
# ifndef BUG1
	if( idebug >= 3 ) printf( "beginit(), curid = %x\n", curid );
# endif
...
instk( id, t, d, s, off ) OFFSZ off; TWORD t; {
	...
	for(;;){
# ifndef BUG1
		if( idebug ) printf( "instk((%x, %x,%x,%x, %x)\n", id, t, d, s, off );
# endif
...
# ifndef BUG1
	if( idebug ) printf( "endinit(), inoff = %x\n", inoff );
# endif
...
# ifndef BUG1
	if( ddebug ){
		printf("\tnonunique entry for %s from %x to %x\n",
			q, *idindex, i );
		}
# endif
	*idindex = i;
	{register unsigned char ii;			/* MFM */
	for( ii=1; ii<=NCHNAM; ++ii ){ /* copy name */
		if( *p++ = *q /* assign */ ) ++q;
		}
	}
	return ( sp );
...
checkst(lev){
	...
	for( i=0, p=stab; i<SYMTSZ; ++i, ++p ){
		...
		else if( p->slevel > lev ) cerror( "%.*s check at level %x", 
						  NCHNAM, p->sname, lev );
		}
	}
#endif
...
struct symtab *
relook(p) register struct symtab *p; {
	/* look up p again, and see where it lies */

	register struct symtab *q;

	/* I'm not sure that this handles towers of several hidden definitions
	   in all cases */
...
# ifndef BUG1
			if (ddebug) printf("removing %.*s from stab[ %x], flags %x level %x\n",
...
hide( p ) register struct symtab *p; {
	...
	if( hflag ) werror( "%.*s redefinition hides earlier one", NCHNAM, 
				p->sname );
# ifndef BUG1
