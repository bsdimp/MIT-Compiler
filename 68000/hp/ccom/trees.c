...
# ifndef BUG1
	if( bdebug ) printf( "buildtree( %s, %x, %x )\n", opst[o], l, r );
# endif
	opty = optype(o);
	...
	if( actions & OTHER ){
		...
		case STREF:
			...
			i = r->tn.rval;
			if( i<0 || i>SYMTSZ+1 || ((sp= &stab[i])->sclass != MOS && sp->sclass != MOU && !(sp->sclass&FIELD)) ){
...
# ifndef BUG1
	if( ddebug > 1 ) printf( "chkstr( %.*s(%x), %x )\n", NCHNAM, 
				stab[i].sname, i, j );
# endif
...
conval( p, o, q ) register int o; register NODE *p, *q; {
...
# endif
	...
	if( off != 0 )
	   p = clocal( block( PLUS, p, makety(offcon( off
#ifndef mc68000
							, t, d, s
#endif
							),INT,0,INT), t, d, s ) );
...
# ifndef BUG1
	if( tdebug ) printf( "tymatch(%x): %x %s %x => %x\n",p,t1,opst[o],t2,tu );
# endif
...
NODE *
block( o, l, r, t, d, s ) NODE *l, *r; TWORD t; {
