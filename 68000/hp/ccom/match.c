...
tshape( p, shape ) register NODE *p; register shape; {	/* MFM */
		...
		for( ;; ++n ){
			if( (mlop = n->mlnode.op) < OPSIMP ){
				switch( optype(mlop) ){
					...
					default:
						cerror("(1)unknown opcode: %x",mlop);
					case BITYPE:
						goto binary;
					case UTYPE:
						break;
		...
		for( ;; ++n ){ /* for each node in multi-level template */
			...
			if( (mlop = n->mlnode.op) < OPSIMP ){
				switch( optype(mlop) ){
					...
					default:
						cerror("(2)unknown opcode: %x",mlop);
					case BITYPE:
						goto binary;
					case UTYPE:
						st = st->left;
