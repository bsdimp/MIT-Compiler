...
clocal(p) register NODE *p; {
	...
	*/
	...
	case NAME:
		/* for NAME nodes, rval has the index into the stab */
	...
	case PCONV:
		...
		pleft = p->in.left;		/* MFM */
		ml = pleft->in.type;		/* MFM */
		if( ( ml==CHAR || ml==UCHAR || ml==SHORT || ml==USHORT ) && pleft->in.op != ICON ) {
		  p->in.op = SCONV;
		  break;
		}
	...
	case SCONV:
		...
		ml = (pleft->in.type == FLOAT || pleft->in.type == DOUBLE );
		if( m != ml ) break;
		...
		m = p->in.type;
		ml = pleft->in.type;
		if( pleft->in.op == ICON ){ /* simulate the conversion here */
			CONSZ val;
			val = pleft->tn.lval;
			switch( m ){
			case CHAR:
...
incode( p, sz ) register NODE *p; register sz; {	/* MFM */
...
ecode( p ) NODE *p; {
	...
	printf(".%d\t%s\n", lineno, ftitle);	/* added for 2 pass */
	prtree( p );				/* added for 2 pass */
