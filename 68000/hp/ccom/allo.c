...
short busy[REGSZ];				/* MFM */

short maxa, mina, maxb, minb;			/* MFM */
...
allo( p, q ) register NODE *p; struct optab *q; {	/* MFM */

	register n;					/* MFM */
	register short i, j;				/* MFM */
...
freereg( p, n ) register NODE *p; {			/* MFM */
...
ushare( p, f, r ) register NODE *p; {		/* MFM */
...
reclaim( p, rw, cookie ) register NODE *p; register int rw, cookie; {/* MFM */
	register NODE **qq;
	register NODE *q;
	register i;
	NODE *recres[5];
...
ncopy( q, p ) register NODE *p, *q; {			/* MFM */
