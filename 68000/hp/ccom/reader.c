...
mainp2( argc, argv ) char *argv[]; {
	...
	if( files ){
		...
		freopen( argv[files++], "r", stdin );	/* MFM */
		}
...
prcook( cookie ){
	...
	if( cookie & SPECIAL ){
		...
		else printf( "SPECIAL+%x", cookie & ~SPECIAL );
		return;
		}
...
short callflag;						/* MFM */
int fregs;				/* # available type A scratch regs */
...
short negrel[] = { NE, EQ, GT, GE, LT, LE, UGT, UGE, ULT, ULE } ;  /* negatives of relationals */ /* MFM */
...
eprint( p, down, a, b ) NODE *p; int *a, *b; {
	...
	*a = *b = down+1;
	...
	case STARG:
	case STASG:
		printf( " size=%x", p->stn.stsize );
		printf( " align=%x", p->stn.stalign );
		break;
		}
