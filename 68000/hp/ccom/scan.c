...
short 	asm_esc; 	/* asm escaped used in file */
short	echoflag;	/* do we echo input to output? */

/* added to support listing of sourcelines as comments in assembly output */
#define GETCHAR	((echopt<echoend) ? (*(echopt++) = getchar()):getchar())
#define UNGETC(x)	(echopt--, ungetc(x, stdin))
...
mainp1( argc, argv ) int argc; char *argv[]; {  /* control multiple files */
	...
	register  short i;
	register char *cp;
	extern int idebug, bdebug, ddebug;			/* MFM */
	extern short edebug, tdebug, xdebug;			/* MFM */
...
# ifndef ONEPASS				/* MFM */
	...
	ejobcode( nerrors ? 1 : 0 );	/* in code.c */
	return(nerrors?1:0);
...
char lxmatch;  /* character to be matched in char or string constant *//* MFM */
