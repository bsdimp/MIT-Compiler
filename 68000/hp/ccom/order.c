...
sucomp( p ) register NODE *p; {
	...
	if( ty == LTYPE ) {
		...
		case UNARY STCALL:
			...
			/* I don't know why all regs are needed during a call
			   if they are saved and restored (except for D0) within
			   the normal prolog and epilog of the function. 
			   Perhaps an investigation of this point would make
			   for better code - MFM
			*/
			return;
	...
	if( o == CALL || o == STCALL ){
		...
		p->in.su = fregs;
		/* This is similar to the above CALL,STCALL stuff. A test
		   to see if all regs are really needed MFM */
		return;
		}
...
genargs( p) register NODE *p; {
	...
	if( p->in.op == STARG ){ /* structure valued argument */
		...
		for( ; size>0; size -= inc ){
			inc = (size>2) ? 4 : 2;
			p->tn.lval -= inc;
			expand( p, RNOP,(inc==4)?"\tmove.l\tAR,Z-\n":"\tmove.w\tAR,Z-\n" );
			}
		reclaim( p, RNULL, 0 );
		return;
		}
