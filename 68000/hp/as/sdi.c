...
/* static char *sdi = "~|^`s.sdi.c R1.7 on 6/27/80"; */ 	/* MFM */
...
register struct oper *op;	/* the operand of the sdi */
int gleng;		/* the length of the most general form of the sdi */
register long base;	/* origin of the the initial span of the sdi, to be */
...
register struct blist *bounds;	/* list of lengths & bounds for the sdi */
{
	register struct sdi *s, **p;
	extern struct csect *Cur_csect;

	if (op->flags_o&O_COMPLEX)	/* not a simple address *//* MFM */
		return(gleng);				          /* MFM */
 ...
 */
longsdi(s, delta)
register struct sdi *s;				/* MFM */
register int delta;
{
	register struct sdi *t;
	register long loc;			/* MFM */
...
register long int offset;			/* MFM */
{
	register struct sdi *s;
	register int total = 0;			/* MFM */

	for (s = sdi_list; s; s = s->sdi_next) {	/* MFM */
		if (csect == s->sdi_csect) {
			if (offset <= s->sdi_loc)
				break;
			total += s->sdi_leng - s->sdi_bounds->b_length;
		/* i.e. increment total by difference between assumed shortest
			length and calculated actual length. */
		}
	}
	return(total);
}
