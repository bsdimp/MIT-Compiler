#include "mical.h"
#include "inst.h"
#include <globaldefs.h> /* repository for universal mask defs and sizes */
...
{	register int i;		/* gen purpose index */
	...
	if (Cur_csect==Text_csect && (Dot&1) &&		/* MFM */
	   (opindex != i_even) && (opindex != i_byte)) Prog_Error(E_ODDADDR);
...
/* miscellaneous classes */
	...
	case i_stop:	stop_op(0x4E72); break;	/* MFM */
	...
	case i_text:	New_Csect(Text_csect); goto pseudonoop;
	case i_data:	New_Csect(Data_csect); goto pseudonoop;
	case i_bss:	New_Csect(Bss_csect); goto pseudonoop;
	case i_globl:	Globl(); goto pseudo;
	case i_comm:	Comm(); goto pseudo;
	case i_even:	Even(); goto pseudonoop;
	case i_ascii:	Ascii(0); goto pseudo;
	case i_asciz:	Ascii(1); goto pseudo;
	case i_align:	Align(); goto pseudo;
	case i_refr:	Globl();Refr(); goto pseudo;
	case i_refa:	Globl();Refa(); goto pseudo;
	case i_include:	Include();goto pseudo;

	pseudonoop:	if (numops) Prog_Error(E_NUMOPS);
...
move_op(opr, size)
	register int opr, size;			/* MFM */
{
	register struct oper *op1, *op2;
	register int r1, r2;			/* MFM */
...
register int opr;	/* normal operand field */	/* MFM */
int iopr;	/* operand if immediate */
register int size;	/* B, W, or L for the size of the operation *//* MFM */
int quick;	/* 1 => quick immediate instruction */
{
	register struct oper *op1, *op2;
	register int r1, r2;			/* MFM */
	register int flag;	/* use to analyze addressing modes */ /* MFM */
...
cbrnch(opr)
{
	...
	if (numops != 1) Prog_Error(E_NUMOPS);
		...
		if (opp->flags_o & O_COMPLEX)	/* not a simple address */
			...
			Code_length = makesdi(opp, 4 /*6*/, Dot + 2, /* MFM */
			 sdi_bound(2, -128L, 127L, 0));		     /* MFM */
			  /* sdi_bound(4, -32768L, 32767L, 0))); */  /* MFM */
		else if (opp->sym_o == 0	/* absolute address */
		 || opp->sym_o->csect_s != Cur_csect /* not in the same csect */
		 || offs < -32768L || offs > 32767L)	/* offset too large */
			baround(opr);
...
regmem(opr)
{
	...
	register int sr, dr;			/* registers */ /* MFM */
...
{
	register struct oper *op1, *op2;
	register int ar;	/* address register */ /* MFM */
...
{
	register struct oper *op1, *op2;
	register int dr;	/* data register */	/* MFM */
...
movem_op(opr)
{
	...
	{
		if (op2->type_o==t_predec) op1->value_o = Switch(op1->value_o);
		rel_val(op1, W);
		eaddr(op2, W);
	}
...
moveq(opr)
{
	...
	op2 = &operands[1];
	r2 = (int)op2->value_o;
	if (!(((op1->value_o & ~0xff) == 0) || ((~op1->value_o & ~0xff) == 0)))
		Prog_Error(E_CONSTANT);
	WCode[0] = opr | ((r2 & 07) << 9) | ((short)op1->value_o & LOBYTE);
	if (numops != 2) Prog_Error(E_NUMOPS);
...
trap(opr)
{
	register struct oper *opp = operands;
	if (numops != 1) Prog_Error(E_NUMOPS);
	if ((opp->type_o != t_immed) || (opp->value_o > 0xf))	/* MFM */
		Prog_Error(E_OPERAND);
	WCode[0] = opr | (((char)opp->value_o) & 0xf);
}
...
{
	register int reg = (int)opptr->value_o;		/* MFM */
	switch(opptr->type_o)
	{
	case t_reg:
	...
	case t_defer:
		if (areg(reg))
			WCode[0] |= (reg & 07) | 020;	/* MFM */
		else Prog_Error(E_REG);
		break;
	case t_postinc:
		if (areg(reg))
			WCode[0] |= (reg & 07) | 030;	/* MFM */
		else Prog_Error(E_REG);
		break;
	case t_predec:
		if (areg(reg))
	...
	case t_displ:
		if (areg(opptr->reg_o))
		{
			if (opptr->sym_o != NULL)
			{
			  if (opptr->sym_o->attr_s & S_CANTDEFINE) Prog_Error(E_IADDR);  /* can't build address */
			  if (opptr->sym_o->attr_s & S_RELATIVE)
			  {
				if (opptr->sym_o == NULL) opptr->disp_o -= 2;
				else opptr->disp_o -= (Dot+2);	/* fix pc rel -- acs */
				WCode[0] |= 073;
			...
			WCode[0] |= (opptr->reg_o & 07) | 050; /*otherwise*/
			rel_val(opptr, W);    /* install displacement */
		}
		else if (pcreg(opptr->reg_o))
		{
			if (opptr->sym_o == NULL) opptr->disp_o -= 2;
			else opptr->value_o -= (Dot+2);	/* fix pc rel -- acs */
			WCode[0] |= 072;
			rel_val(opptr, W);	/* install displacement */
		}
		else if (dreg(opptr->reg_o))
		{
		    	if (opptr->sym_o != NULL) /* handle bad d reg displ */
				{
				if (!(opptr->sym_o->attr_s & S_RELATIVE)) Prog_Error(E_REG);
				}
		        else Prog_Error(E_REG);
			if (opptr->sym_o == NULL) opptr->disp_o -= 2;
			else opptr->disp_o -= (Dot+2);	/* fix pc rel -- acs */
	...
	case t_normal:
		if (opptr->sym_o != NULL)
		  if (opptr->sym_o->attr_s & S_RELATIVE)
		  {	
			WCode[0] |= 073;	/* ADDED FOR REFR -- ANNY */
			opptr->value_o -= (Dot+2); 	/* fix pc rel -- acs */
			rel_val(opptr,W);	/* install addr */
			break;
		  }
	case t_absl:
		WCode[0] |= 071;
...
register struct oper *opptr;
{
	register int indexval = 0;			/* MFM */
	register int reg = (int)opptr->value_o;		/* MFM */

	if (areg(reg) || dreg(reg))			/* MFM */
		indexval = reg << 12;
	else Prog_Error(E_REG);
	if (opptr->flags_o == O_LINDEX) indexval |= 0x0800;	/* else word */
	if (opptr->disp_o >= -0200 && opptr->disp_o < 0200)
	  indexval |= (opptr->disp_o & LOBYTE);
...
register struct oper *op;
{
	...
	case t_normal:
		if (op->sym_o != NULL)             /* FOR REFR SYMS -- acs */
			if (op->sym_o->attr_s & S_RELATIVE) return(0);
	default:
		return(1);
	}
}
...
register int mask;
{
	...
	if (mask & 256) val |= 128;	/* a0 */	

	if (mask & 512) val |= 64; 	/* a1 */

	if (mask & 1024) val |= 32;	/* a2 */

	if (mask & 2048) val |= 16;	/* a3 */

	if (mask & 4096) val |= 8;	/* a4 */

	if (mask & 8192) val |= 4;	/* a5 */

	if (mask & 16384) val |= 2;	/* a6 */

	if (mask & 38768) val |= 0;	/* a7 */

	if (mask & 1) val |= 32768; 	/* d0 */

	if (mask & 2) val |= 16384;	 /* d1 */

	if (mask & 4) val |= 8192;	/* d2 */

	if (mask & 8) val |= 4096;	/* d3 */

	if (mask & 16) val |= 2048;	/* d4 */

	if (mask & 32) val |= 1024;	/* d5 */

	if (mask & 64) val |= 512;	/* d6 */

	if (mask & 128) val |=256;	/* d7 */
